...
from vakyartha.base import Document
...
class LanceDBDocumentStore(BaseDocumentStore):
    ...
    def add(
        ...
        **kwargs,
        ...
        if self.collection_name not in self.db_collection.table_names():
            if data :
               ...
               document_collection = self.db_collection.open_table(self.collection_name)
               if data :
    ...
    def query(
        ...
        return [
            Document(
                 ...
                 metatdata = json.loads(doc["attributes"]),
            )
            for doc in docs
        ]
    ...
    def delete(self,ids:Union[List[str],str],refresh_indices : bool = True):
        ...
        document_collection = self.db_connection.open_table(self.collection_name)
        id_filter = ", ".join([f"'{id}'" for _id in ids])
        query_filter = f"id in ({id_filter})"
